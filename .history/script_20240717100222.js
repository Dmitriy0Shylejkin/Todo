...
dom.tasks.addEventListener('input', (event) => {
  const target = event.target;
  const isTaskTextEL = target.classList.contains('todo__task-text');

  if (isTaskTextEL && editableTask === target) {
    const map = {
      '&': '&amp;',
      '<': '&lt;',
      '>': '&gt;',
    ...
    const escapedText = target.textContent.replace(/[&<>"'#%:?*]/g, (m) => map[m]);
    target.textContent = escapedText;
  }
});
...
function updateTaskText(id, newText, list) {
  ...
  const escapedText = newText.replace(/[&<>"'#%:?*]/g, (m) => map[m]);
  list.forEach((task) => {
    if (task.id == id) {
      task.text = newText;
    }
