...
dom.clearCompleted.onclick = () => {
    const newTasks = tasks.filter((task) => !task.isComplete); //фильтрует массив задач, оставляя только не выполненные
    tasks = newTasks;
    tasksRender(tasks);
}
}
//dom.clearCompleted.onclick = () => {
    ...
    tasks = newTasks;
    tasksRender(tasks);
}//фильтрует массив задач, оставляя только не выполненные