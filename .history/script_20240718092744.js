...
// function addNewTask() {
//   const newTaskText = dom.new.value.trim().replace(/\s+/g, ' ');
//   if (newTaskText && !tasks.some(task => task.text === newTaskText)) {
//     tasks.push({ id: Date.now(), text: newTaskText, isComplete: false });
//     dom.new.value = '';
//     tasksRender(tasks);
//     dom.new.focus();
...
// dom.new.onkeyup = (event) => event.key === 'Enter' && addNewTask();
...
//   dom.tasks.innerHTML = currentTasks.map(({ id, text, isComplete }) => `
//     <div id="${id}" class="todo__task ${isComplete ? 'todo__task_complete' : ''}">
//       <label class="todo__checkbox">
//         <input type="checkbox" ${isComplete ? 'checked' : ''}>
//         <div class="todo__checkbox-div"></div>
//       </label>
//       <div class="todo__task-text" contenteditable="false">${text}</div>
//       <div class="todo__task-del">-</div>
//     </div>
//   `).join('');
//   dom.pagination.innerHTML = [...Array(totalPages)].map((_, i) => `
//     <button class="pagination__page ${i === currentPage ? 'pagination__page--active' : ''}" data-page="${i + 1}">${i + 1}</button>
//   `).join('');
...
//   dom.notCompletedCount.innerHTML = list.filter(({ isComplete }) => !isComplete).length;
//   dom.completedCount.innerHTML = list.filter(({ isComplete }) => isComplete).length;
// }
...
//       filteredTasks = tasks.filter(({ isComplete }) => !isComplete);
...
//   const task = list.find(({ id: taskId }) => taskId === id);
//   if (task) task.isComplete = !task.isComplete;
// }

// function deleteTask(id, list) {
//   const taskIndex = list.findIndex(({ id: taskId }) => taskId === id);
//   if (taskIndex !== -1) list.splice(taskIndex, 1);
// }

// dom.markAll.onclick = () => {
//   const isAllTasksCompleted = tasks.every(({ isComplete }) => isComplete);
//   tasks.forEach((task) => task.isComplete = !isAllTasksCompleted);
//   tasksRender(tasks);
// }

// dom.clearCompleted.onclick = () => {
//   tasks = tasks.filter(({ isComplete }) => !isComplete);
//   tasksRender(tasks);
...
//   const cleanedTaskText = newText.trim().replace(/\s+/g, ' ');
//   if (cleanedTaskText) {
//     const task = list.find(({ id: taskId }) => taskId === id);
//     if (task) task.text = cleanedTaskText;
//     tasksRender(tasks);
//   } else {
//     alert('Поле не может быть пустым или содержать только пробелы.');
//   }
